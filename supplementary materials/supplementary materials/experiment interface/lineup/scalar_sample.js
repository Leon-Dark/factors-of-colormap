...
function ScalarSample(w, h, canvas, model, colormap)
{
    ...
    // Do initial visualization after everything is set up
    if (this.canvas && this.field.getColorMap()) {
        this.vis();
    }
}
...
ScalarSample.prototype.vis = function() {
  ...
  //  console.log('visualizing');
    // Get the field data and colormap
    const data = this.field.view;
    const colormap = this.field.getColorMap();
    const w = this.w;
    const h = this.h;

    if (!colormap || typeof colormap.mapValue !== 'function') {
    ...
    // Draw pixels directly using fillStyle and fillRect
    for (let i = 0; i < h; i++) {
        for (let j = 0; j < w; j++) {
            const value = data[i * w + j];
            if (isNaN(value)) {
                this.ctx.fillStyle = 'rgba(0,0,0,1)';
            } else {
                const color = colormap.mapValue(value);
