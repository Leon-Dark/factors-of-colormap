		...
		for (var i=0, len=colorMap.length; i<len-1; i++)
		{
			...
			if (colorMap[i].lab && colorMap[i+1].lab)
			{
				...
				else if (interpType == 'hsl')
				{
					interpolator = d3.interpolateHcl(
						d3.hcl(lab1[0], lab1[1], lab1[2]),
						d3.hcl(lab2[0], lab2[1], lab2[2])
					);
				}
				else
				{
			...
			else
			{
				...
				else if (interpType == 'hsl')
               {
                  ...
                  interpolator = d3.interpolateHcl(
                     d3.hcl(rgb1[0], rgb1[1] , rgb1[2]),
                     d3.hcl(rgb2[0], rgb2[1] , rgb2[2])
                  );
               }
				else
				{
...
ColorMap.prototype.sampleRamp = function(n)
{
	...
	return Object.assign(new Array(n), colors);
	...
	else
	{
		...
		for (var i=0, len=interpolators.length; i < len; i++)
		{
			...
			if (range[0] <= v && range[1] >= v)
			{
				...
				// 添加调试信息
				//console.log("插值器类型:", typeof interp);
				//console.log("插值器名称:", interp.name);
				//console.log("插值器字符串表示:", interp.toString());
...
function getColorPreset(preset, m0, m1, brandNew)
{
	...
	// 检查预设是否存在
	if (!colorsets[preset]) {
		console.error("预设颜色映射不存在:", preset);
		console.log("可用的颜色映射:", Object.keys(colorsets));
		
		// 使用第一个可用的颜色映射作为后备
		var availablePresets = Object.keys(colorsets);
		if (availablePresets.length > 0) {
			console.warn("使用第一个可用的颜色映射代替:", availablePresets[0]);
			preset = availablePresets[0];
		} else {
			console.error("没有可用的颜色映射");
			return null;
		}
	}
    ...
    var specialInterpolation = 'rgb';

	if (!colorScheme)
	{
		console.error("Could not find preset colors: " + preset);
		return null;
	}
	else
	{
		...
		if (Array.isArray(colorScheme))
			...
			//	//console.log(preset);
			//	//console.log(preloaded.colormap);
